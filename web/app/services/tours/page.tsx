...
import { MapPin, Clock, Users, Landmark, Crown, Globe } from 'lucide-react';
...
function TourCard({ tour, index }: { tour: typeof palaceTours[0] | typeof museumTours[0]; index: number }) {
    return (
        <GlassCard delay={index * 0.08}>
            ...
            {'highlight' in tour && (
                <div className="inline-block px-2 py-1 rounded-md bg-[#d4af37]/10 border border-[#d4af37]/20 text-[#d4af37] text-xs mb-3">
                    {(tour as any).highlight}
                </div>
            )}
            <p className="text-gray-400 text-sm mb-4 leading-relaxed">{tour.desc}</p>
